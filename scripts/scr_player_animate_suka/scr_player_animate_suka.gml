/// @function scr_player_animate_suka
function scr_player_animate_suka()
{
	...
	switch animation
	{
		...
		case ANI_WAIT:
			...
			if super_timer > 0
			{
				ani_start(spr_suka_idle_super, 18, 0, 0, [0, 1, 2, 1]);
				break;
			}
			...
			var _duration =
			[
				180, 12, 12, 18, 18, 18, 18, 18, 18, 18, 18, 60
			];
			
			ani_start(spr_suka_idle, _duration, 0, 3, _order);
		...
		case ANI_MOVE:
			...
			else if abs(spd_ground) >= 6
			{
				...
				{
					_sprite = spr_suka_run;
				}
				else
				{
					_sprite = spr_suka_dash;
				}
			}
		...
		case ANI_SPIN:
			...
			ani_start(spr_suka_spin, _duration, 0, 0, [0, 4, 1, 4, 2, 4, 3, 4]);
			ani_update_duration(_duration);
		...
		case ANI_PUSH:
			...
			var _sprite = super_timer > 0 ? spr_sonic_push_super : spr_suka_push;
		...
		case ANI_DUCK:
			...
			ani_start(spr_suka_duck, 4, 0, 1);
			
		break;
		
		case ANI_LOOKUP:
			ani_start(spr_suka_lookup, 4, 0, 1);
		break;
		...
		case ANI_HURT:
			ani_start(spr_suka_hurt);
		break;
		
		case ANI_DEATH:
			ani_start(spr_suka_death);
		break;
		
		case ANI_DROWN:
			ani_start(spr_suka_drown);
		break;
		
		case ANI_SKID:
			ani_start(spr_suka_skid, 8, 0, 1);
		break;
		...
		case ANI_BREATHE:
			ani_start(spr_suka_breathe, 24);
		break;
		
		case ANI_BOUNCE:
			ani_start(spr_suka_bounce, [56, 600]);
		break;
